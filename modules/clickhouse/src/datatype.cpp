...
static std::vector<uint8_t> getBytes(Nemea::UnirecRecordView& record, ur_field_id_t fieldID)
{
	...
	result.reserve(arr.size());
	std::transform(arr.begin(), arr.end(), std::back_inserter(result), [](std::byte value) {
...
static std::vector<uint8_t> getMac(Nemea::UnirecRecordView& record, ur_field_id_t fieldID)
{
	...
	std::copy(std::begin(mac.mac.bytes), std::end(mac.mac.bytes), std::back_inserter(result));
	return result;
}
...
static std::vector<uint64_t> getTimeArr(Nemea::UnirecRecordView& record, ur_field_id_t fieldID)
{
	...
	std::transform(
		...
		timeArr.end(),
		std::back_inserter(result),
		[](const Nemea::UrTime& value) -> uint64_t {
			return (static_cast<uint64_t>(ur_time_get_sec(value.time)) * nsecInSec)
...
static bool isArr(ColumnType type)
{
	return type < ColumnType::INT8;
}

static std::shared_ptr<clickhouse::Column> makeArrColumn(ColumnType type)
{
...
static ColumnWriterFn makeArrColumnwriter(ColumnType type)
{
