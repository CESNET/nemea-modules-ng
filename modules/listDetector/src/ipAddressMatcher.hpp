 ...
 * @brief Declaration of the IpAddressMatcher class for keeping and matching IP prefixes against IP
 * addresses
 *
...
public:
	...
	/**
	 * @brief Finds IP prefixes mathing given IP address.
	 * @param address The IP adress to match prefixes against.
	 * @return Bitset where matching IP prefix indexes are set to true.
	 */
	std::vector<bool> getMatchingIpRulesMask(const Nemea::IpAddress& address) const noexcept;
