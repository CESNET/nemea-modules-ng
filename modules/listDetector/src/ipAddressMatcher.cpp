 ...
 * @brief Implementation of the IpAddressMatcher class for keeping and matching IP prefixes against
 * IP addresses
 *
 * SPDX-License-Identifier: BSD-3-Clause
 */

#include "ipAddressMatcher.hpp"
...
void IpAddressMatcher::addPrefix(const IpAddressPrefix& prefix) noexcept
{
...
uint16_t IpAddressMatcher::insertNode(const OctetNode& node, NodePos pos) noexcept
{
...
std::vector<bool>
IpAddressMatcher::getMatchingIpRulesMask(const Nemea::IpAddress& address) const noexcept
{
	std::vector<bool> matchingBitset(m_lastInsertIndex);
	checkOctet(address, 0, {0, m_octets[0].size()}, matchingBitset);
	return matchingBitset;
}

uint16_t IpAddressMatcher::getNotLastNodeNextNode(NodePos pos) const noexcept
{
...
void IpAddressMatcher::checkOctet(
	const Nemea::IpAddress& address,
	uint8_t octetIndex,
	const std::pair<uint16_t, uint16_t>& searchRange,
	std::vector<bool>& matchingBitset) const noexcept
{
...
bool IpAddressMatcher::ipMatchesNetworkOctet(const Nemea::IpAddress& address, NodePos pos)
	const noexcept
{
