...
class alignas(CACHE_LINE_SIZE) TimeoutBucket {
public:
	...
	insert(const uint64_t key, const Value& value, const TimeType& currentTime)
	{
		...
		bool isFound = std::any_of(
			m_keys.begin(),
			m_keys.end(),
			[this, &key, &sameKeyIndex, &currentTime, index = 0](const auto& bucketKey) mutable {
				if (!isValid(index)) {
					index++;
					return false;
				}
		...
		if (isFull()) {
			std::size_t victimIndex = getVictimIndex();
...
private:
	...
	std::size_t getVictimIndex() const noexcept
	{
		...
		return std::distance(m_expirationTime.begin(), minIt);
	}
	...
	std::array<uint8_t, 39> m_padding;
	// cache line 1
